...
function handleDataApi(request, response) {
  if (request.method === "GET") {
    fs.readFile(dataFile, "utf8", (error, text) => {
      ...
      if (error) {
        sendJson(response, 500, { error: "Unable to read finance-data.json." });
        return;
      }
      try {
        sendJson(response, 200, { exists: true, data: JSON.parse(text) });
      } catch (parseError) {
        sendJson(response, 500, { error: "finance-data.json is not valid JSON." });
      }
    });
    return;
  }
    ...
    readRequestBody(request, (readError, body) => {
      ...
      try {
        ...
        fs.writeFile(tempFile, output, "utf8", (writeError) => {
          if (writeError) {
            sendJson(response, 500, { error: "Unable to write finance-data.json." });
            return;
          }
          fs.rename(tempFile, dataFile, (renameError) => {
            if (renameError) {
              sendJson(response, 500, { error: "Unable to save finance-data.json." });
              return;
            }
...
function listen(port, attemptsLeft) {
  ...
  server.listen(port, () => {
    ...
    console.log(`Finance Planner Desktop is running at ${url}`);
